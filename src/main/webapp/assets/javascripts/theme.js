...
(function (theme, $) {
  ...
  if (typeof $.browser !== 'undefined' && $.browser.mobile && $html.hasClass(
          'fixed') /*[[*/ && $(window).width() < 768 /*]]*/) {
...
//		customScroll: ( !Modernizr.overflowscrolling && !isAndroid && $.fn.nanoScroller !== 'undefined'),
    ...
    eventsSidebarRight: function () {
      ...
      var close = function (e) {
        if (!!e && !!e.target && ($(e.target).closest('.sidebar-right').get(0)
    ...
    eventsContentMenu: function () {
      ...
      var close = function (e) {
        if (!!e && !!e.target && !$(e.target).closest(
                '.inner-menu-collapse').get(0) && ($(e.target).closest(
                '.inner-menu').get(0) || !$(e.target).closest('html').get(0))) {
          return false;
        }
  ...
  Mailbox.prototype = {
    ...
    eventsCompose: function () {
      ...
      var adjustComposeSize = function () {
        ...
        contentBodyPaddingBottom = parseInt($contentBody.css('paddingBottom'),
            10) || 0;
...
(function ($) {
  ...
  $(function () {
    ...
    .on('click', '.panel-actions a.fa-times', function (e) {
      ...
      if (!!( $panel.parent('div').attr('class') || '' ).match(
              /col-(xs|sm|md|lg)/g) && $panel.siblings().length === 0) {
  ...
  GMapBuilder.prototype = {
    ...
    build: function () {
      ...
      if (!!window.SnazzyThemes) {
        ...
        $.each(window.SnazzyThemes, function (i, theme) {
          themeOpts.push($('<option value="' + theme.id + '">' + theme.name
              + '</option>').data('json', theme.json));
        });
      ...
      google.maps.event.addDomListener(window, 'load', function () {
        ...
        _self.map = new google.maps.Map(_self.$mapContainer.get(0),
    ...
    events: function () {
      ...
      this.$previewModal.on('hidden.bs.modal', function () {
        _self.$previewModal.find('iframe').get(
    ...
    updateMapProperty: {
      ...
      latlng: function () {
        ...
        lng = this.$wrapper.find(
            '[data-builder-field][name="longitude"]').val();
      ...
      maptype: function (value) {
        ...
        mapStyles = this.$wrapper.find(
            '[data-builder-field="maptheme"]').children('option').filter(
            ':selected').data('json');
        ...
        if ($.inArray(google.maps.MapTypeId[value.toUpperCase()],
                ['terrain', 'roadmap']) > -1 && !!mapStyles) {
      ...
      maptheme: function (value) {
        ...
        mapType = google.maps.MapTypeId[this.map.getMapTypeId() === 'terrain'
            ? 'TERRAIN' : 'ROADMAP'];
        options = {};
        json = this.$wrapper.find('[data-builder-field="maptheme"]').children(
            'option').filter(':selected').data('json');
    ...
    _onGeocodeResult: function (marker, response, status) {
      ...
      if (!response || status !== google.maps.GeocoderStatus.OK) {
        ...
        marker.lat = Math.round(result.geometry.location.lat() * 1000000)
            / 1000000;
        marker.lng = Math.round(result.geometry.location.lng() * 1000000)
            / 1000000;
    ...
    preview: function () {
      ...
      previewHtml = [
        ...
        '</style>',
        '<div id="' + this.$wrapper.find('[data-builder-field="mapid"]').val()
      ...
      customScript.text = "window.initialize = function() { " + this.generate()
          + " init(); }; ";
    ...
    generate: function () {
      ...
      var zoomControl = this.$wrapper.find(
          '[data-builder-field="zoomcontrol"] option:selected').val()
          !== 'false';
      var mapTypeControl = this.$wrapper.find(
          '[data-builder-field="maptypecontrol"] option:selected').val()
          !== 'false';
      var overviewMapControl = this.$wrapper.find(
          '[data-builder-field="overviewcontrol"] option:selected').val().toLowerCase();
      var $themeControl = this.$wrapper.find(
          '[data-builder-field="maptheme"] option:selected').filter(
          ':selected');
      ...
      .replace(/\{lng\}/,
          this.$wrapper.find('[data-builder-field][name="longitude"]').val())
      .replace(/\{zoom\}/, this.$wrapper.find(
          '[data-builder-field="zoomlevel"] option:selected').val())
      .replace(/\{zoomControl\}/, zoomControl)
      .replace(/\{disableDoubleClickZoom\}/, this.$wrapper.find(
          '[data-builder-field="clicktozoomcontrol"] option:selected').val()
          === 'false')
      .replace(/\{mapTypeControl\}/, mapTypeControl)
      .replace(/\{scaleControl\}/, this.$wrapper.find(
          '[data-builder-field="scalecontrol"] option:selected').val()
          !== 'false')
      .replace(/\{scrollwheel\}/, this.$wrapper.find(
          '[data-builder-field="scrollwheelcontrol"] option:selected').val()
          !== 'false')
      .replace(/\{panControl\}/, this.$wrapper.find(
          '[data-builder-field="pancontrol"] option:selected').val()
          !== 'false')
      .replace(/\{streetViewControl\}/, this.$wrapper.find(
          '[data-builder-field="streetviewcontrol"] option:selected').val()
          !== 'false')
      .replace(/\{draggable\}/, this.$wrapper.find(
          '[data-builder-field="draggablecontrol"] option:selected').val()
          !== 'false')
      .replace(/\{overviewMapControl\}/, overviewMapControl !== 'false')
      .replace(/\{mapTypeId\}/, this.$wrapper.find(
          '[data-builder-field="maptype"] option:selected').val().toUpperCase());

      if (zoomControl) {
        work = {
          zoomControlOptions: {
            style: this.$wrapper.find(
                '[data-builder-field="maptypecontrol"] option:selected').val().toUpperCase()
          }
        };
        output = output.replace(
            /\{zoomControlOptions\}/, "zoomControlOptions: {\r\n                style: google.maps.ZoomControlStyle."
            + this.$wrapper.find(
                '[data-builder-field="zoomcontrol"] option:selected').val().toUpperCase()
            + "\r\n\            },");
      } else {
    ...
    build: function () {
      ...
      if (!$('html').hasClass('no-csstransitions') && $(window).width() > 767) {
        ...
        $el.appear(function () {
          ...
          delay = ($el.attr('data-appear-animation-delay') ? $el.attr(
              'data-appear-animation-delay') : self.options.delay);
...
(function (theme, $) {
  ...
  PluginCarousel.prototype = {
    ...
    build: function () {
      this.options.wrapper.owlCarousel(this.options).addClass(
          "owl-carousel-init");
...
(function (theme, $) {
  ...
  PluginChartCircular.prototype = {
    ...
    build: function () {
      ...
      shouldAnimate = $.isFunction($.fn['appear']) && ( typeof $.browser
          !== 'undefined' && !$.browser.mobile );
...
(function (theme, $) {
  ...
  PluginPortlet.prototype = {
    ...
    saveState: function (panel) {
      ...
      var collapse = panel.find('.panel-actions').children(
          'a.fa-caret-up, a.fa-caret-down'),
    ...
    events: function ($el) {
      ...
      $el.find('.panel-actions').on('click',
          'a.fa-caret-up, a.fa-caret-down, a.fa-times', function (e) {
            setTimeout(function () {
              _self.saveState($el);
            }, 250);
          });
  ...
  PluginToggle.prototype = {
    ...
    events: function ($el) {
      ...
      $el.find('> label').click(function (e) {
        ...
        if (self.options.isAccordion && typeof(e.originalEvent)
            != 'undefined') {
      ...
      if ($.isFunction($.fn.sortable)) {
        this.$el.sortable({
          sort: function (event, ui) {
            var top = event.pageY - _self.$el.offset().top
  ...
  WidgetToggleExpand.prototype = {
    ...
    events: function () {
      ...
      $toggler.on('click.widget-toggler', function () {
        _self.$el.hasClass('widget-collapsed') ? _self.expand(_self.$el)
